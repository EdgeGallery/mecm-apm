...
# db parameters validation
if [ ! -z "$APM_DB_USER" ] ; then
   validate_name "$APM_DB_USER"
   valid_name="$?"
   if [ ! "$valid_name" -eq "0" ] ; then
...
if [ ! -z "$APM_DB_HOST" ] ; then
   validate_host_name "$APM_DB_HOST"
   valid_db_host_name="$?"
...
if [ ! -z "$APM_DB_PORT" ] ; then
   validate_port_num "$APM_DB_PORT"
   valid_apm_db_port="$?"
   if [ ! "$valid_apm_db_port" -eq "0" ] ; then
      echo "invalid apm db port number"
      exit 1
   fi
fi

validate_password "$APM_DB_PASSWORD"
valid_apmdb_password="$?"
if [ ! "$valid_apmdb_password" -eq "0" ] ; then
   echo "invalid apmdb password, complexity validation failed"
   exit 1
fi
